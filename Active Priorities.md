...
- [ ] **VLC sub-project 2: swipe to seek, test on the Surface.** Build 3 (bubble, jump on release) tested OK 09-26. Build 4 (https://github.com/badboyfarrxxx-ship-it/vlc-build/releases/tag/build-9e59d4b38-run4-a1) shows the picture live while dragging, and speed follows the finger: 30 s per screen width slow, up to 600 s for a fast flick. Tested OK on the Surface 09-26. Open issue: VLC may get stuck when it closes (leftover processes that only a restart clears); being checked. Later: camera wall in VLC, then video editing. See [[VLC build pipeline]].
