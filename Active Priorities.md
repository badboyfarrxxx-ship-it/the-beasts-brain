...
### Open Tasks
- [ ] **SECURITY, NOW: stop the malware on the Surface.** `Windows Helper.exe` is still running and beaconing to `global-stash.beer`. End it in Task Manager, delete `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\Windows Helper.lnk`, then remove the four Defender exclusions and the Allowed-threat entry it planted. See [[Security incident 2026-09-12]].
