...
- [ ] **HP: install the NVIDIA graphics driver.** The HP (GeForce 710A) has no GPU driver, so Chrome falls back to Windows' software renderer (confirmed 09-22). It needs NVIDIA's R470 branch (a few hundred MB download). Then re-run the Undermoot test file on it. See [[Building tiny11 images]] and [[colony-game]].
