...
- [ ] **Rebuild VLC 4 through the new build pipeline.** Spec written 09-25 and waiting on Nathan's review; then an implementation plan, then build. The installed "VLC 4.0 dev" crashes on launch on the rebuilt Surface (cause not found). Later sub-projects: swipe to seek, camera wall in VLC, video editing. See [[VLC build pipeline]].
