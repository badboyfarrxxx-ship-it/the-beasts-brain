...
- [ ] **VLC sub-project 2: swipe to seek.** Already built into VLC 4 (confirmed on the Surface 09-26): a one-finger drag on the video left or right jumps 3 s per step, up or down changes volume. Waiting on Nathan: keep as is, or tune it (bigger jumps, a time bubble while dragging, speed that follows the swipe). Later: camera wall in VLC, then video editing. See [[VLC build pipeline]].
