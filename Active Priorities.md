...
- [ ] **Finish the VLC 4 test on the Surface: play a video.** It opens as of 09-26 (the "won't start" was a stale skins2 interface setting; old settings moved to `%APPDATA%\vlc-old-<date>`). Once a video plays: mark [[Building VLC for Windows]] retired and start sub-project 2, swipe to seek. See [[VLC build pipeline]].
