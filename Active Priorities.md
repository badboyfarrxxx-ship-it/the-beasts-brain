...
- [ ] **HP: install the NVIDIA graphics driver.** The HP (GeForce 710A) has no GPU driver, so Chrome falls back to Windows' software renderer (confirmed 09-22). It needs NVIDIA's R470 branch (a few hundred MB download). Before that, run the new Undermoot test file (`Documents/undermoot-bench.html`, rebuilt 09-22 with the missing-driver notice) once, to see the notice on real hardware; then install the driver and run it again. See [[Building tiny11 images]] and [[colony-game]].
