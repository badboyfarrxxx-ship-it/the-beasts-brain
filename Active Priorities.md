...
- [ ] **SECURITY: re-isolate `E:`.** The partition holding the File History copy of the old profile (known-bad repacks and the cracks lure) is mounted again: it was remounted on purpose 09-16 for its scan and never dismounted, and this never made it onto this list. Checked 09-22: volume `{12920787-...}` is live at `E:`. Fix, from an elevated prompt: `mountvol E: /P`. Also still Nathan's call from the same scan: what to do with the thousands of low-confidence hits in `E:`'s `Compressed files`, `Other files` and `apk mobile apps` (leave, move, or wipe). See [[Machine drives]] and [[Security incident 2026-09-12]].
