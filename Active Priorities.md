...
- [ ] **Test the new VLC 4 build on the Surface.** Built 09-25 on GitHub (repo `vlc-build`). Uninstall the old "VLC 4.0 dev" from Settings > Apps, download the zip from https://github.com/badboyfarrxxx-ship-it/vlc-build/releases/tag/build-9e59d4b38-run2-a1, unzip it, open the `vlc-4.0.0-dev` folder and run `vlc.exe` (Windows may warn it's unsigned: More info > Run anyway), then play a video. If it crashes, run the crash checks in [[VLC build pipeline - implementation plan]] Task 6. Once it works: mark [[Building VLC for Windows]] retired and start sub-project 2, swipe to seek. See [[VLC build pipeline]].
