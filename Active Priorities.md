...
- [ ] **Rebuild VLC 4 through the new build pipeline.** Plan approved 09-25; scripts and workflow built and reviewed in a local repo. **Waiting on Nathan to create the private `vlc-build` repo on GitHub** (Claude can't); then push, first build, and his test on the Surface. The installed "VLC 4.0 dev" crashes on launch on the rebuilt Surface (cause not found). Later sub-projects: swipe to seek, camera wall in VLC, video editing. See [[VLC build pipeline]].
