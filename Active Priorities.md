...
- [ ] **Point [[camera-wall]] at the real cameras.** The app is built and tested against fake
  cameras. Three steps left, all on the camera side: install ffmpeg
  (`winget install Gyan.FFmpeg`), switch RTSP on in each camera's phone app, then
  `npm run discover` and `npm run check` to fill in `cameras.json`. Ring, Blink, Nest and
  Arlo cannot be streamed locally at all and get a link tile instead.
