...
- [ ] **SECURITY: decide how isolated `E:` needs to be.** The partition holding the File History copy of the old profile (known-bad repacks and the cracks lure) had been mounted since 09-16. Nathan ran `mountvol E: /P` on 09-22 and it went fully offline, but **the reboot at 8:06 AM brought it back online without a drive letter**: nothing can browse to it as `E:`, but anything that uses its volume ID path (`\\?\Volume{12920787-...}\`) can still read it (checked 09-22, 8:15 AM). So `/P` survives a reboot as "no letter" only, not "offline". Open: accept no-letter as enough, or go further. Also still Nathan's call from the same scan: what to do with the thousands of low-confidence hits in `E:`'s `Compressed files`, `Other files` and `apk mobile apps` (leave, move, or wipe). See [[Machine drives]] and [[Security incident 2026-09-12]].
- [ ] Get this computer set up as a machine for building programs.
- [ ] Start making money from the programs I build.
- [ ] **tiny11: boot the second computer from the stick and install.** The image is built, verified, and written to the 14.9 GB USB stick (`G:` while attached), which has also been verified part by part. The stick carries `ChromeSetup.exe` and the instructions. Two things to know before booting: it is **UEFI-only**, so a BIOS-only or CSM machine will not see it and it would need redoing with Rufus; and keep the new machine offline or metered, because 21H2 will pull several GB of catch-up updates. Check the target's network adapter is covered by in-box drivers before wiping it. The target is the HP; Nathan was given the install steps on 09-15, but the result was never recorded, so confirm whether it's done. See [[Building tiny11 images]].
...
- [ ] **my-agent: open The Beasts Brain in Obsidian.** Obsidian 1.13.7 was installed 09-21 but has never been launched, so the vault is not registered. Open it once and choose Open folder as vault on `C:\Users\Fredy 2\The Beasts Brain`.
