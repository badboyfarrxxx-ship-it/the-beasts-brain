...
> **Retired (2026-09-26).** VLC is now built on GitHub by [[VLC build pipeline]], confirmed working on the Surface. This MSYS2 method is kept for the history and the MSYS2 fixes. The old installer's "crash on launch" on the rebuilt machine was most likely a stale `intf=skins2` setting in `%APPDATA%\vlc\vlcrc`, not the build. Kept for the history and the MSYS2 fixes.
