...
- **Verified on the Surface (2026-09-26).** The full Qt interface opens and plays video. Sub-project 1 is done; next is swipe to seek.
...
- **Swipe to seek patch (2026-09-26):** `patches/0001-touch-scrub-seek.patch` changes `modules/gui/qt/player/qml/Player.qml`. A sideways one-finger drag shows a time bubble (target time and offset) and jumps there on release. The offset is 300 s x (drag / width) squared, set by `fullSwipeSeconds`. Up/down drags still control volume. Canceled drags and non-seekable media don't seek. Built as run 3 (https://github.com/badboyfarrxxx-ship-it/vlc-build/releases/tag/build-9e59d4b38-run3-a1); Nathan tested it and it worked.
- **Swipe to seek, live version (2026-09-26):** Nathan tested run 3 (bubble and jump worked) and asked to see the picture while dragging and for speed to follow the finger. The patch now seeks live, at most every 100 ms. Each movement covers 30 s per screen width when slow, up to 600 s per width for a fast flick (`slowSecondsPerWidth`, `fastSecondsPerWidth`, using Qt's smoothed finger velocity). It stops just short of the end so a drag can't end playback. Built as run 4 (https://github.com/badboyfarrxxx-ship-it/vlc-build/releases/tag/build-9e59d4b38-run4-a1); Nathan tested it and it works.
- **Stuck VLC processes (2026-09-26):** 11 VLC processes from earlier runs were stuck in the background with no window. They locked files, stopped new copies from opening, and couldn't be killed; a restart cleared them. Cause found with a minidump: after VLC's normal shutdown, Windows unloads `libqt_plugin.dll` and a Qt cleanup routine waits forever for a thread that is already gone. Not the graphics driver, sound or network. Fix: `patches/0002-exit-without-dll-teardown.patch` ends `vlc.exe` right after shutdown (settings saved), skipping the unload. Built as run 5 (https://github.com/badboyfarrxxx-ship-it/vlc-build/releases/tag/build-9e59d4b38-run5-a1); tested on the Surface, VLC now exits within seconds. Sub-project 2 is done; next is the camera wall. Check with `Get-Process vlc` in PowerShell.
- **Missing plugin cache:** the zip has no `plugins\plugins.dat`, so VLC scans every add-on at each start. Run `vlc-cache-gen.exe plugins` once in the VLC folder after unzipping.
