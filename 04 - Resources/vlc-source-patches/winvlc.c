                    ...
                    int nCmdShow )
{
    ...
    /***
     * LoadLibrary* calls from the modules and 3rd-party code search
     * System32, the application directory, any AddDllDirectory() path,
     * and (for a DLL loaded by full path) that DLL's own directory.
     * The application dir is needed when plugins are linked against a
     * *shared* toolchain library set (e.g. a system Qt) rather than the
     * static contrib libs the official build uses. cwd stays excluded
     * (SetDllDirectory above) and System32 stays first (below).
     * */
