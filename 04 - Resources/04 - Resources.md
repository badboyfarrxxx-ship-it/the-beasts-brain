...
- [[Building VLC for Windows]]: the old way VLC was compiled on the Surface itself (MSYS2 + prebuilt contribs), with every off-the-supported-path fix recorded. Retired 2026-09-26, replaced by [[VLC build pipeline]].
...
- [[VLC build pipeline]]: design spec and status for building VLC 4 on GitHub's build servers as a portable Windows zip, replacing the MSYS2 method. Built 2026-09-25 (repo `vlc-build`), verified on the Surface 2026-09-26. Sub-project 1 of 4 (then swipe to seek, camera wall, video editing).
